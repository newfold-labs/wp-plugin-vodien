...
const OldPostsComments = ({ setError, notify }) => {
	...
	const disableCommentsNoticeTitle = () => {
		return disableCommentsOldPosts
			? __('Disabled old post comments', 'wp-plugin-vodien')
			: __('Enabled old post comments', 'wp-plugin-vodien');
	};

	const disableCommentsNoticeText = () => {
		return disableCommentsOldPosts
			? __('Comments on old posts are disabled.', 'wp-plugin-vodien')
			: __('Comments are allowed on old posts.', 'wp-plugin-vodien');
	};
...
const CloseCommentsDays = ({ setError, notify }) => {
	...
	const closeCommentsDaysNoticeTitle = () => {
		return (
			__('Comments setting saved ', 'wp-plugin-vodien')
		);
	};
	...
	const closeCommentsDaysNoticeText = () => {
		...
		return (
			__('Comments on posts are disabled after ', 'wp-plugin-vodien') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-vodien')
		);
	};
	...
	const closeCommentsDaysLabelText = () => {
		...
		return (
			__('Close comments after ', 'wp-plugin-vodien') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-vodien')
		);
	};
...
const CommentsPerPage = ({ setError, notify }) => {
	...
	const commentsPerPageNoticeTitle = () => {
		return __('Comments setting saved.', 'wp-plugin-vodien');
	};

	const commentsPerPageNoticeText = () => {
		//`Posts will display ${commentsPerPage} comments at a time.`
		return (
			__('Posts will display ', 'wp-plugin-vodien') +
			commentsPerPage +
			_n(
				' comment at a time.',
				' comments at a time.',
	...
	return (
		<SelectField
			...
			label={__('Display ', 'wp-plugin-vodien') + commentsPerPage +
				__(' comments per page.', 'wp-plugin-vodien')}
...
const CommentSettings = () => {
	...
	return (
		<Container.SettingsField
			title={__('Comments', 'wp-plugin-vodien')}
			description={__('Comments allow visitors to provide feedback and respond to your posts or pages.', 'wp-plugin-vodien')}
		>
