...
const PerformanceFeatureSettings = () => {
	...
	const getPerformanceNoticeTitle = () => {
		return performance
			? __( 'Performance Enabled', 'wp-plugin-vodien' )
			: __( 'Performance Disabled', 'wp-plugin-vodien' );
	};
	...
	const getPerformanceNoticeText = () => {
		return performance
			...
			: __( 'Performance will no longer display.', 'wp-plugin-vodien' );
	};
	...
	const notifyError = () => {
		notify.push( 'feature-toggle-notice', {
			title: __( 'Sorry, that is not allowed.', 'wp-plugin-vodien' ),
			description: __(
	...
	useUpdateEffect( () => {
		...
		updateUI( '.wppv-app-navitem-Performance', performance );
	}, [ performance ] );
