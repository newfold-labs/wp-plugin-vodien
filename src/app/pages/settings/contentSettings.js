...
const ContentRevisions = ({ setError, notify }) => {
	...
	const contentRevisionsNoticeTitle = () => {
		return (
			__('Post revision setting saved ', 'wp-plugin-vodien')
		);
	};

	const contentRevisionsNoticeText = () => {
		return (
			__('Posts will save  ', 'wp-plugin-vodien') +
			contentRevisions +
			_n(' revision.', ' revisions.', parseInt(contentRevisions), 'wp-plugin-vodien')
		);
	};

	const contentRevisionsDescriptionText = () => {
		return (
			__('Saving drafts and updating published content creates revisions. Make changes with confidence, knowing you can take ', 'wp-plugin-vodien') +
			contentRevisions +
			_n(' step back.', ' steps back.', parseInt(contentRevisions), 'wp-plugin-vodien')
		);
	};
	...
	return (
		<SelectField
			...
			label={__('Number of revisions posts can save ', 'wp-plugin-vodien')}
...
const EmptyTrash = ({ setError, notify }) => {
	...
	const emptyTrashNoticeTitle = () => {
		return (
			__('Trash setting saved ', 'wp-plugin-vodien')
		);
	};

	const emptyTrashNoticeText = () => {
		return (
			__('The trash will automatically empty every ', 'wp-plugin-vodien') +
			numTrashWeeks +
			_n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-vodien' )
		);
	};
	...
	return (
		<SelectField
			...
			label={__('Trash emptying frequency ', 'wp-plugin-vodien')}
			description={
			__('The trash will automatically empty every ', 'wp-plugin-vodien') +
			numTrashWeeks +
			 _n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-vodien' )
...
const ContentSettings = () => {
	...
	return (
		<Container.SettingsField
			title={__('Content Options', 'wp-plugin-vodien')}
			description={__('Controls for content revisions and how often to empty the trash.', 'wp-plugin-vodien')}
		>
