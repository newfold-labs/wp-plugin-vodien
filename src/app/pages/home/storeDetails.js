...
const hasRequiredStoreInfo = () =>
	Boolean(
		window?.NFDStoreInfo?.data?.address && window?.NFDStoreInfo?.data?.city
	);
...
const StoreDetails = () => {
	...
	const titleText =
		siteKind === 'store'
			? __( 'Congrats, your store is live!', 'wp-plugin-vodien' )
			: __( 'Congrats, your website is live!', 'wp-plugin-vodien' );
	const detailsButtonText = hasStoreInfo
		? __( 'Store Details', 'wp-plugin-vodien' )
		: __( 'Add Store Details', 'wp-plugin-vodien' );

	useEffect( () => {
		const handleStoreInfoChange = () =>
			setHasStoreInfo( hasRequiredStoreInfo() );
