...
const HelpCard = ({ item }) => {
	return ( 
		<Card className={`wppv-help-card card-help-${item.name}`}>
			<Card.Content>
				<Title 
					as="h3" 
					size="4"
...
const Help = () => {
	...
	return (
		<Page className={"wppv-app-help-page"}>
			<Container className={'wppv-app-help-container'}>
				<Container.Header>
					<Title as="h2" className="nfd-flex nfd-items-center nfd-gap-2">
						<QuestionMarkCircleIcon className="nfd-w-8 nfd-h-8" />
						{__('Help', 'wp-plugin-vodien')}
					</Title>
					<span>{__('We are available 24/7 to help answer questions and solve your problems.', 'wp-plugin-vodien')}</span>
				</Container.Header>
