...
export const webSettingsApiFetch = ( data, passError, thenCallback ) => {
	return apiFetch( {
		// path: 'vodien/v1/settings', //  can't use path bacause it breaks on temp domains
		url: NewfoldRuntime.createApiUrl( '/vodien/v1/settings' ),
		method: 'POST',
		data,
	} )
		.then( ( response ) => {
...
export const webPurgeCacheApiFetch = ( data, passError, thenCallback ) => {
	return apiFetch( {
		url: NewfoldRuntime.createApiUrl( '/vodien/v1/caching' ),
		method: 'DELETE',
		data,
	} )
		.then( ( response ) => {
...
export const addUtmParams = (url, params = {}) => {
	params.utm_source = `wp-admin/admin.php?page=vodien${window.location.hash}`;
	params.utm_medium = 'vodien_plugin';
