 ...
 * @return bool
 */
function vodien_has_plugin_install_date() {
	return ! empty( get_option( 'vodien_plugin_install_date', '' ) );
}
 ...
 * @return string
 */
function vodien_get_plugin_install_date() {
	return (string) get_option( 'vodien_plugin_install_date', gmdate( 'U' ) );
}
 ...
 */
function vodien_set_plugin_install_date( $value ) {
	update_option( 'vodien_plugin_install_date', $value, true );
}
 ...
 * @return int
 */
function vodien_get_days_since_plugin_install_date() {
	return absint( ( gmdate( 'U' ) - vodien_get_plugin_install_date() ) / DAY_IN_SECONDS );
}

/**
 * Basic setup
 */
function vodien_setup() {
	...
	if ( ! vodien_has_plugin_install_date() ) {
		$date = false;
		if ( ! empty( $install_date ) ) {
			$date = \DateTime::createFromFormat( 'M d, Y', $install_date );
		}
		vodien_set_plugin_install_date( $date ? $date->format( 'U' ) : gmdate( 'U' ) );
	}
}

add_action( 'admin_init', __NAMESPACE__ . '\\vodien_setup' );
 ...
 * @return int
 */
function vodien_install_date_filter( $install_date ) {
	return vodien_get_plugin_install_date();
}
add_filter( 'nfd_install_date_filter', __NAMESPACE__ . '\\vodien_install_date_filter' );
